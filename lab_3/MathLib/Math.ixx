...
public:
	...
	Complex(double re) {//конструктор преобразовани€
		m_re = re; m_im = 0.0;
	}
...
export int FindGreatestCommonDivisor(int a, int b) {//Ќаибольший общий делитель
	a = abs(a);
	b = abs(b);
	if (b > a) {
		int temp = b;
...
export int FindLeastCommonMultiple(int a, int b) {//Ќаименьшее общее кратное
	return abs(a * b) / FindGreatestCommonDivisor(a, b);
}

export class Rational {//дробь
private:
	int m_nominator;//верх
	int m_denominator;//низ
	void reduction() {//функци€ дл€ сокращени€ дробей
...
export Complex f(const Complex& z) {
	int a_real = 0, a_imag = 1;//?? - да
	Complex a(a_real, a_imag);
	Complex result = a - z*sin(double(2*z));
	return result;
}
...
export Rational f(const Rational& r) {
	...
	Rational result = a - r*sin(double(2*r));
	return result;
}
